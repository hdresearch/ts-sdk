...
/**
 * SSH Client for connecting to VMs over TLS.
 *
...
export class SSHClient {
  ...
  /**
   * Start keep-alive timer (10s interval, 6 max missed)
   */
  ...
  private async uploadDirectory(
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        const result = await this.uploadDirectory(sftp, lp, rp, options);
        filesTransferred += result.filesTransferred;
        bytesTransferred += result.bytesTransferred;
      } else {
        const result = await this.uploadFile(sftp, lp, rp, options);
        filesTransferred += result.filesTransferred;
        bytesTransferred += result.bytesTransferred;
      }
    }
  ...
  private async downloadDirectory(
    ...
    for (const entry of entries) {
      const rp = `${remoteDir}/${entry.filename}`;
      const lp = path.join(localDir, entry.filename);

      if (entry.attrs.isDirectory()) {
        const result = await this.downloadDirectory(sftp, rp, lp, options);
        filesTransferred += result.filesTransferred;
        bytesTransferred += result.bytesTransferred;
      } else {
        const result = await this.downloadFile(sftp, rp, lp, options);
        filesTransferred += result.filesTransferred;
        bytesTransferred += result.bytesTransferred;
      }
    }
